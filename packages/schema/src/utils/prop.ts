import { DecoratorKeys, PropType } from '../enums/$.js';
import { CannotBeSymbolError } from '../errors/$.js';
import type {
	ArrayPropOptions,
	BasePropOptions,
...
export function processProp(input: ProcessPropOptions): void {
	...
	let Type: any | undefined = Reflect.getMetadata(
		DecoratorKeys.Type,
		target,
		key
	);
	...
	assertion(
		typeof key === 'string',
		() => new CannotBeSymbolError(name, key)
	);
...

	{
		...
		switch (propKind) {
			...
			case PropType.ARRAY:
				...
				if (
					('ref' in rawOptions || 'refPath' in rawOptions) &&
					!('type' in rawOptions)
				) {
					Type = undefined;
				}
	...
	const modelOptionsOfType: IModelOptions =
		Reflect.getMetadata(DecoratorKeys.ModelOptions, Type ?? {}) ?? {};

	// throw a error when both "discriminators" as a prop-option and as a model-option are defined
	if (
		'discriminators' in rawOptions &&
		!utils.isNullOrUndefined(modelOptionsOfType?.options?.discriminators)
	) {
		throw new DuplicateOptionsError([
			'discriminators(prop-option)',
			'discriminators(model-option)'
		]);
	}
	...
	if (
		...
		const discriminatorsToUse =
			rawOptions?.discriminators ?? modelOptionsOfType?.options?.discriminators;
		logger.debug('Found option "discriminators" in "%s.%s"', name, key);
		const gotType = utils.getType(discriminatorsToUse, true);
		utils.assertion(
			gotType.dim === 1,
			() =>
				new OptionDoesNotSupportOptionError(
					'discriminators',
					'dim',
					'1',
					`dim: ${gotType.dim}`
		...
		const discriminators: DiscriminatorObject[] = (
			...
			if (typeof val === 'object') {
				if (!('type' in val)) {
					throw new Error(
						`"${name}.${key}" discriminator index "${index}" is an object, but does not contain the "type" property!`
					);
				}
			...
			throw new Error(
				`"${name}.${key}" discriminators index "${index}" is not an object or an constructor!`
			);
		});
	...
	if ('ref' in rawOptions) {
		...
		utils.assertion(
			...
			() => new RefOptionIsUndefinedError(name, key)
		);
	...
	if (utils.isWithVirtualPOP(rawOptions)) {
		...
		const virtuals: VirtualPopulateMap = new Map(
			Reflect.getMetadata(DecoratorKeys.VirtualPopulate, target.constructor) ??
				[]
		);
		virtuals.set(key, rawOptions);
		Reflect.defineMetadata(
			DecoratorKeys.VirtualPopulate,
			virtuals,
			target.constructor
		);
	...
	if (Type instanceof Passthrough) {
		logger.debug(
			'Type is "instanceof Passthrough" ("%s.%s", %s, direct: %s)',
			name,
			key,
			propKind,
			Type.direct
		...
		switch (propKind) {
			case PropType.ARRAY:
				schemaProp[key] = utils.mapArrayOptions(
					rawOptions,
					newType,
					target,
					key
			...
			case PropType.NONE:
				...
				return;
			default:
				throw new InvalidPropTypeError(
					propKind,
					name,
					key,
					'PropType(Passthrough)'
				);
		}
	}

	// use "Type" if it is an suitable ref-type, otherwise default back to "ObjectId"
	const refType = utils.isAnRefType(Type)
		? Type
	...
	if (!utils.isNullOrUndefined(rawOptions.enum)) {
		...
		if (
			rawOptions.enum?.constructor === Object &&
			'values' in rawOptions.enum
		) {
		...
		let { dim: enumDim, type: enumType }: { dim: number; type: any } =
			...
			if (Type === String || Type === mongoose.Schema.Types.String) {
				...
				enumType = Object.entries<string | number>(enumType) // get all key-value pairs of the enum
					...
					.filter(([enumKey, enumValue], _i, arr) => {
						...
						if (
							utils.isNullOrUndefined(enumValue) ||
							arr.findIndex(([k]) => k === enumValue.toString()) <= -1
						) {
							// if there is no reverse mapping, throw an error
							throw new NotNumberTypeError(
								name,
								key,
								enumKey,
								typeof enumValue
...

	{
		...
		if (!utils.isString(Type)) {
			...
			utils.warnNotCorrectTypeOptions(
				name,
				key,
				'String',
				'String-Validate',
	...
	const hasCachedSchema = !utils.isNullOrUndefined(
		Reflect.getMetadata(DecoratorKeys.CachedSchema, Type)
	);
			...
			case PropType.MAP:
				...
				if (utils.isTypeMeantToBeArray(rawOptions)) {
					...
					finalType = utils.mapArrayOptions(
						{ ...mapped.inner, dim: rawOptions.dim },
						Type,
						target,
						key
					);
	...
	switch (propKind) {
		...
		case PropType.MAP:
			...
			if ('dim' in rawOptions) {
				...
				const { type, ...outer } = utils.mapArrayOptions(
					rawOptions,
					virtualSchema,
					target,
					key,
